...
//        int n = 2;
    ...
    public int leastIntervalSLOW(char[] tasks, int n) {
        Map<Integer, Integer> counts = new HashMap<>();
        for (char task : tasks) counts.put(task - 'A', counts.getOrDefault(task - 'A', 0) + 1);

        PriorityQueue<int[]> p = new PriorityQueue<>(Comparator.comparingInt(o -> -o[1]));
        for (Integer c : counts.keySet()) p.offer(new int[]{c, counts.get(c)});

        Queue<int[]> standBy = new LinkedList<>();
        for (int i = 0; i <= n; i++) standBy.offer(null);

        int ans = 0;
        while (counts.size() > 0) {
            ans++;
            int[] tmp = standBy.poll();
            if (tmp != null) p.offer(tmp);
            ...
            if (p.size() > 0) {
                ...
                if (tmp[1] == 0) {
                    standBy.offer(null);
                    counts.remove(tmp[0]);
                } else standBy.offer(tmp);
            } else standBy.offer(null);
        }
